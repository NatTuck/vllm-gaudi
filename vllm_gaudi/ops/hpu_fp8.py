...
# EXPERIMENTAL fused MoE router: resolved ONCE at import (outside dynamo tracing)
# so the op lib is loaded before any layer region is compiled. Keeping the
...
if _HPU_MOE_GATHER:
    try:
        ...
        _root = _Path(__file__).resolve().parents[4]  # workspace root (…/FastQwen)
...
class HPUFp8MoEMethod(Fp8MoEMethod):
    ...
    def apply_monolithic(
        ...
        **kwargs,
        ...
        elif _HPU_ROUTER_FUSED:
            # Fused softmax+topk custom op (single kernel instead of ~47
            # torch.topk launches). topk_ids int32 -> upcast below. Direct,
            # graph-capturable call (op lib loaded at module import).
            topk_ids, topk_weights = _router_select_op(router_logits, layer.top_k)
        else:
